...
//        test3();
    ...
    private void test2(HttpServletResponse response) throws IOException {
        ...
        InputStream in = loader.getResourceAsStream("db/config/db3.properties");
        Properties prop = new Properties();
        prop.load(in);
        String driver = prop.getProperty("driver");
        String url = prop.getProperty("url");
        String username = prop.getProperty("username");
        String password = prop.getProperty("password");
        response.getWriter().println("用类装载器读取src目录下的db.config包中的db3.properties配置文件：");
        response.getWriter().println(
                MessageFormat.format(
                        "driver={0},url={1},username={2},password={3}",
                        driver,url, username, password));
    ...
    public void test4() throws IOException {
        ...
        // path=01.avi
        String path = this.getServletContext().getRealPath("/WEB-INF/classes/servletDemo/01.mp4");
        //获取文件名
        String filename = path.substring(path.lastIndexOf("\\") + 1);
        InputStream in = this.getServletContext().getResourceAsStream("/WEB-INF/classes/servletDemo/01.mp4");
